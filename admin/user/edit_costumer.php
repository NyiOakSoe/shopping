...
use LDAP\Result;
  ...
  $id=$_GET['id'];
  $stmt=$pdo->prepare("SELECT * FROM costumer WHERE id=$id");
  $stmt->execute();
  ...
  if(isset($_POST['btn'])){
    if(empty($_POST['name']) || empty($_POST['email'])|| empty($_POST['phone']) || empty($_POST['address'])){
      ...
      $stmt=$pdo->prepare("SELECT * FROM costumer WHERE email=:email AND id!=:id");
      $stmt->execute(
        array(
          ':email'=>$email,
          ':id'=>$id
      ...
      $cos_result=$stmt->fetch(PDO::FETCH_ASSOC);
      //var_dump($admin_result);
      
      if(!$cos_result){
        if(empty($password)){
          $stmt=$pdo->prepare("UPDATE costumer SET name='$name',email='$email',phone='$phone',address='$address' WHERE id=$id ");
       
        }else{
        $stmt=$pdo->prepare("UPDATE costumer SET name='$name',email='$email',phone='$phone',address='$address',password='$password' WHERE id=$id ");
        }
      $result=$stmt->execute();
      if($result){
