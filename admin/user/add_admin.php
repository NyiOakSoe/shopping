<?php
  ...
  if(isset($_POST['btn'])){
    if(empty($_POST['name']) || empty($_POST['email'])|| empty($_POST['phone']) || empty($_POST['address']) || empty($_POST['password'])){
      ...
      if(!$result){
          $stmt=$pdo->prepare("INSERT INTO admin(name,email,phone,address,password)VALUES(:name,:email,:phone,:address,:password) ");
      $result=$stmt->execute(
          array(
              ':name'=>$name,
              ':email'=>$email,
      ...
      if($result){
          echo "<script>alert('New Admin added');window.location.href='user.php'</script>";
      }
      }else{
          echo "error";
      }
